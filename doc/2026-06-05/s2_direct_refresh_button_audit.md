...
95% 확신 기준 구현안은 기존 `관리자에게 S2 최신화 요청` 버튼을 제거하지 않고, 명시적으로 허용된 관리자 환경에서만 `관리자 직접 S2 최신화` 패널을 추가하는 것이다.

## 조사 결과

- `app.py`에는 이미 `run_s2_refresh()`, `run_s2_guard_refresh()`, `run_s2_service_content_refresh()`, `run_s2_full_replace()`가 있다.
- 현재 사이드바 버튼은 `create_s2_refresh_request_task()`만 호출해서 ClickUp 요청 task를 만든다.
...
- 기존 사용자 흐름 회귀: ClickUp 요청 버튼은 유지한다.
